...
from fineman.helper_functions import reweight_graph, super_source_bfd, compute_reach, transpose_graph, b_hop_sssp, b_hop_stsp
from fineman.independent_set_or_crust import find_is_or_crust
from scripts import generate_double_tree


def _eliminate_independent_set(graph, independent_set):
...
def _compute_price_functiom_to_make_U_r_remote(graph, neg_edges, negative_sandwich, beta):
...
def _subgraph_with_out_set(graph: dict[int, dict[int, int]], out_set: set):
    new_graph = {}
    new_neg_edges = set()
        ...
        case (y,U):
            ...
            match find_is_or_crust(graph_T, neg_edges_T, U, c, c+1):
                ...
                case I:
                    ...
                    return phi_1, _eliminate_independent_set(graph_phi1, I)
...
def main():
    tree_graph, neg_edges = generate_double_tree(5, -10)

    result = elimination_algorithm(tree_graph, neg_edges)
